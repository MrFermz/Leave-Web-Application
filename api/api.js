...
async function createuser(req, res, body) {
    ...
    if (token) {
        ...
        // database query
        let sql         =   'INSERT INTO users (empID, firstname, lastname, nickname, username, password, departmentID, typeID, approverID) VALUES ?'
        let values      =   [[
        ...
        db.query(sql, [values], async function (error, result) {
            ...
            if(error) {
                ...
                let sqlLeaveDays    =   `UPDATE users SET leaveDaysID = ${leaveDaysID} WHERE UID = ${insertId}`
...
async function userapprlist(req, res) {
    ...
    if (result) {
            ...
            let sql     =   'SELECT approver.approverID, users.username \
                            FROM approver \
...
async function getleavedays(req, res) {
    ...
    if (token) {
        ...
        let sql     =   `SELECT leaveDaysID FROM users WHERE UID = ${token.id}`
...
async function createleave(req, res, body) {
    ...
    if (token) {
        ...
        let sql     =   `INSERT INTO leaves (leaveType, dateStart, dateEnd, reasons, status, UID) VALUES ?`
        let values  =   [[
...
async function getleavelist(req, res) {
    ...
    if (token) {
        ...
        let sql =   `SELECT leaves.leaveID, leaves.leaveType, leaves.dateStart, leaves.dateEnd, leaves.reasons, leaves.status, users.nickname, users.empID
                     FROM leaves
...
async function approve(req, res, body) {
    ...
    if (token) {
        ...
        let sql     =   `UPDATE leaves
                         SET status = 1, dateApprove = '${data.dateApprove}'
...
async function getuserslists(req, res) {
    ...
    if (token) {
        ...
        let sql     =       `SELECT users.UID, users.firstname, users.lastname, users.nickname, users.departmentID, users.typeID, users.approverID
                             FROM users`
