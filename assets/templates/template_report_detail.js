var headers     = ['#', 'id', 'name', 'sick', 'vacation', 'business', 'substitution']
        ...
        <div id="card-report-main">
            ...
            ${content.length > 0 
                ? `<table id="table-users" class="table-report">
                        <thead>
                            <tr id="tr-header">
                                ${headers.map((ele, i) => { return `<th id="th-${i}" class="th-${ele}">${ele.charAt(0).toUpperCase() + ele.slice(1)}</th>` }).join("")}
                            </tr>
                        </thead>
                        <tbody>
